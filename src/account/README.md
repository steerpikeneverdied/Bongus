...
**Invariants**
- The account is `@fortis/bishop-meta-contract`'s six-section blob — `resources` (id-keyed wallet) /
...
- The SAME blob shape + patch/apply vocabulary + idempotent transaction protocol are what a meta
  ...
  the `@fortis/bishop-meta-client` npm package) and moving its validation to the server.
