...
import { resolve } from '../assets.js';   // emoji fallbacks sourced from the registry, not literals
...
export function throwChest(from, to, { img = null, emoji = resolve('ui.chest').emoji, onLand } = {}) {
  const el = document.createElement('div');
...
export function spendBurst(from, { emoji = resolve('ui.coin').emoji, img = null, color = CU.spend.color } = {}, count = CU.spend.count) {
