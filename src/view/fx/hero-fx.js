...
export function fxEquipBest(tileEl, slotEls, fromPow, toPow) {
  ...
  setTimeout(() => {
    ...
    const c = centerOf(tileEl);
    particleBurst(c.x, c.y, false);
    // Stat increase floats on the HERO TILE (not the gear slots — those only get the swap-flash above).
    const gain = Math.max(0, Math.round(toPow - fromPow));
    if (gain > 0) floatText(c.x, c.y, '+' + gain + ' PWR', 'stat pwr', 0);
  }, after);
}
...
// Power-increase feedback fired ALONGSIDE the tile fx when you level/equip from the Heroes screen — on
// the matching IN-COMBAT hero chip (Autobattler) + on the squad-power total. Both no-op when the target
// isn't on screen (combat panel hidden behind a full screen, etc.).
export function fxCombatPow(cid, gain) {
  if (gain <= 0) return;
  const chip = document.querySelector(`.hero-chip[data-battle-hero="${cid}"]`);
  if (!chip) return;
  const powEl = chip.querySelector('.hero-pow') || chip;
  powEl.animate([{ transform: 'scale(1)' }, { transform: 'scale(1.35)', offset: 0.4 }, { transform: 'scale(1)' }], { duration: 520, easing: 'cubic-bezier(.2,1.4,.3,1)' });
  const c = centerOf(powEl);
  floatText(c.x, c.y - 6, '+' + fmt(Math.round(gain)) + ' PWR', 'stat pwr', 0);
}
export function fxSquadTotal(gain) {
  if (gain <= 0) return;
  const el = document.querySelector('.hs-sqpow');
  if (!el) return;
  el.animate([{ transform: 'scale(1)' }, { transform: 'scale(1.22)', offset: 0.4 }, { transform: 'scale(1)' }], { duration: 520, easing: 'cubic-bezier(.2,1.4,.3,1)' });
  const c = centerOf(el);
  floatText(c.x, c.y - 6, '+' + fmt(Math.round(gain)) + ' PWR', 'stat pwr', 0);
}
