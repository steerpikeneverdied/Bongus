...
// Boss-gate glyph as HTML: the generated icon <img> once exported, else the emoji fallback (so this
// renders identically to before until art is generated). Keeps the ⚠/⚔/☠ flourishes registry-backed.
const gateIco = (id) => { const a = resolve('icon.' + id); return a.img ? `<img class="gate-ico" src="${a.img}" alt="">` : a.emoji; };
const skulls = (n) => { let s = ''; for (let i = 0; i < 5; i++) s += `<span class="sk ${i < n ? 'on' : ''}">${gateIco('skull')}</span>`; return s; };
...
export function showGate(level) {
  ...
  setHTML(`<div class="intro-veil gate"></div>
    <div class="gate-top"><div class="gate-warn">${gateIco('warn')} ${STRINGS.combat.bossAhead} ${gateIco('warn')}</div>
...
async function seqBoss(run, level, zone) {
  ...
  setHTML(`<div class="intro-veil gate"></div><div class="intro-flash"></div>
    <div class="gate-top" id="gt"><div class="gate-warn">${gateIco('warn')} ${STRINGS.combat.bossAhead} ${gateIco('warn')}</div>
      <div class="gate-name">${STRINGS.combat.unknownName}</div>
      <div class="skulls">${skulls(t)}</div>
      <div class="threat">${STRINGS.combat.threat}: ${STRINGS.combat.threatLabels[t]}</div></div>`);
  ...
  setHTML(`<div class="intro-veil gate" style="opacity:1"></div>
    <div class="gate-top" id="gt"><div class="gate-warn reveal">${gateIco('boss')} ${STRINGS.combat.bossReveal} ${gateIco('boss')}</div>
