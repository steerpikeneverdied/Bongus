...
import { resolve, heroAsset, iconAsset } from '../assets.js';
...
export default function ShooterGame({ input, onComplete }) {
  ...
  useEffect(() => {
    ...
    function spawnBoss() { state.boss = { lane: 0, d: 0.66, hp: BOSS.hp, maxHp: BOSS.hp, hitT: 0, dir: 1, t: 0 }; while (state.snipers.length < BOSS.escortSnipers) spawnSniper(); shake = Math.max(shake, 9); floatText(W / 2, H * 0.34, `${iconAsset('boss').emoji} BOSS ${iconAsset('boss').emoji}`, '#ffd9a0', true); }
    ...
    function drawMine(mn) { const d = mn.d; if (d > 1.05) return; const mx = projX(mn.lane, d), my = projY(d), rx = mn.radius * halfW(d), ry = rx * 0.42, pulse = 0.5 + 0.5 * Math.sin(mn.t * 9);
      ...
      const s = Math.max(16, 26 * scaleAt(d)); ctx.font = s + 'px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(iconAsset('minigame.bomb').emoji, mx, my - ry * 0.6); ctx.textBaseline = 'alphabetic'; ctx.restore(); }
    ...
    function drawHUD() {
      ...
      outlineText(iconAsset('minigame.squad').emoji + ' ' + state.squad, 14, 22, '#fff', 16, 'left');
      outlineText(iconAsset('rarity.diamond').emoji + ' ' + state.gems, W - 14, 22, '#8fd6ff', 15, 'right');
      outlineText(iconAsset('minigame.coins').emoji + ' ' + state.coins, W - 14, 42, '#ffd45e', 15, 'right');
