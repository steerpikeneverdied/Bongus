  ...
  useEffect(() => {
    ...
    const spawnFoe = () => {
      ...
      T(() => {
        ...
        disintegrate(cx, cy);
        pop(cx, cy + 28, Math.random() < 0.5 ? iconAsset('skull').emoji : '−' + fmt(30 + Math.floor(Math.random() * 220)), Math.random() < 0.5);
    ...
    <div className="afk-popup">
      ...
      <div className="card">
        ...
        <div className="head">
          ...
          <div className="away"><Art a={resolve('ui.timer')} /> Away for <b>{fmtT(p.ms)}</b></div>
        </div>
        ...
        <div className="scene">
          ...
          <div className="killwrap">
            ...
            <div className="killrow"><Icon id="skull" className="sk" /><span className="killnum ol" ref={killRef}>0</span></div>
          </div>
          {squad.map((ch, i) => {
        ...
        <div className="cta">
          <button className={`claim${claimed ? ' done' : ''}`} disabled={claimed} onClick={doClaim}>
            {claimed ? <><Icon id="check" /> COLLECTED</> : <><img src={resolve('ui.chest.rare').img} alt="" />CLAIM</>}
          </button>
        </div>
      </div>
    </div>
