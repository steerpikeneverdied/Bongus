      ...
      <div className="bcard-body">
        <div className="bcard-top">
          <div className="bcard-head">
            ...
            {banner.timer ? <span className="bcard-timer"><Art a={resolve('ui.timer')} /> {banner.timer}</span> : null}
          </div>
          <div className="bcard-pity-col">
            ...
            {pityEntries.map((p) => (
              <span key={p.rarity} className={`ppill ${p.near ? 'near' : ''}`} style={{ color: p.meta.color }}>
                <Art a={resolve('ui.pity')} className="pg" />
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '12px 12px 0' }}>
      <div className="screen-head" style={{ flex: '0 0 auto' }}>
        <span><Art a={resolve('ui.summonMachine')} /> {STRINGS.screens.summon}</span>
        <span className="pool"><Art a={resolve('ui.coin')} /> {fmt(state.coins)}</span>
      </div>
        ...
        <div className="collection">
          {Object.keys(HEROES).map((hero) => (
            <div key={hero} className={`coll-cell ${owned.has(hero) ? 'owned' : 'locked'}`}>
              {owned.has(hero) ? <Art a={heroAsset(hero)} className="coll-emoji" /> : <Art a={resolve('ui.locked')} className="coll-emoji" />}
            </div>
          ))}
        </div>
      </PeekScroll>
