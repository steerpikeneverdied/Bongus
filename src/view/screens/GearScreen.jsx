    ...
    <div className="gsheet open" role="dialog" aria-label="Gear actions">
      ...
      <div className="gsheet-actions">
        <button type="button" className="gsheet-btn level" disabled={!levelOk} onClick={() => onLevel(g.id)}>
          ...
          {gearAtMax(g) ? null : <span className="cost"><Art a={resolve('ui.gearXp')} /> {gearLevelCost(g.level)}</span>}
        </button>
        <button type="button" className="gsheet-btn fuse" disabled={!fuseOk} onClick={() => onFuse(g.id)}>
          ...
          {nr ? <span className="cost"><Art a={resolve('ui.fuseFodder')} /> {fodder}/{GEAR_FUSE.fodder} · <Art a={resolve('ui.coin')} /> {fmt(cost)}</span> : null}
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '12px 12px 0' }}>
      <div className="rd-head">
        <h2><Art a={resolve('ui.gearXp')} /> {STRINGS.screens.gear}</h2>
        <span className="rd-pool"><Art a={resolve('ui.gearXp')} /> {fmt(state.gearXp)}</span>
      </div>
      {items.length === 0 ? (
