...
import { zoneStartLevel, zoneBossLevel, zoneIndexForLevel, afkRatesForLevel, zoneRecommendedRange } from '../../model/map.js';
...
function ZoneRow({ zi, currentLevel, furthest, innerRef, onPop, onStart }) {
  ...
  const range = zoneRecommendedRange(zi); // recommended power range covering this area's nodes
  ...
  const stateChip = isCur
    ...
    : cleared ? <span className="mtile-state done"><Icon id="check" /> {s.cleared}</span>
      : <span className="mtile-state locked"><Icon id="lock" /></span>;
      ...
      >
        ...
        <div className="mtile-head">
          <div>
            ...
            <div className="mtile-pow"><Icon id="attack" /> {fmtKr(range.min)}–{fmtKr(range.max)}</div>
          </div>
          {stateChip}
        </div>
        <div className="mboss"><Art a={resolve(`enemy.${zone.bossId}`)} className={`mboss-img ${killed ? 'reveal' : 'silh'}`} /></div>
        {locked ? (
          <div className="mreq"><Icon id="lock" /> Reach Lv {a}</div>
        ) : (
