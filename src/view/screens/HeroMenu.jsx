    ...
    <div className="hero-menu" style={rootStyle}>
      ...
      <div className="hm-header">
        <div className="hm-emblems">
          <div className="emb"><span><Icon id="ability.skill" /></span></div><div className="emb-line" />
          {canAscend ? (
          ...
          <div className="emb"><span><Icon id="power" /></span></div>
        </div>
      ...
      <div className="hm-body">
        ...
        <div className="side-col right">
          {[['ability.basic', 'basic'], ['ability.skill', 'normal'], ['ability.limit', 'limit']].map(([ic, k], i) => (
            <button key={k} className="abil" onClick={() => setSkillIdx(i)}>
              <Icon id={ic} className="ai" /><span className="alv">{abilLv}</span>
            </button>
          ))}
        </div>
      ...
      <div className="hm-foot">
        <div className="hm-stats">
          <div className="power-line"><Icon id="power" className="pw-ic" /><span className="pw-val hm-ol" ref={powRef}>{fmt(power)}</span></div>
          <div className="stat-row">
            ...
            <div className="stat"><Icon id="stat.hp" className="ico" /><div className="sv"><b className="hm-ol" ref={hpRef}>{fmt(stats.maxHp)}</b><small>Health</small></div></div>
            <div className="stat"><Icon id="stat.atk" className="ico" /><div className="sv"><b className="hm-ol" ref={atkRef}>{fmt(stats.atk)}</b><small>Attack</small></div></div>
            <div className="stat"><Icon id="stat.def" className="ico" /><div className="sv"><b className="hm-ol" ref={defRef}>{fmt(stats.def)}</b><small>Defense</small></div></div>
          </div>
        </div>
        {applied.length > 0 && (
        ...
        <div className="hm-actions">
          <div className="lvl-stacks">
            <div className="lvl-stack">
              <button className="btn primary" data-ftue="level-up" disabled={atMax && !canLevelHero(char, state.heroXp)} onClick={doLevelUp}>
                {atMax ? <><Icon id="rarity.star" /> MAX LEVEL</> : <>LEVEL UP<small><Art a={resolve('ui.heroXp')} /> {fmt(heroLevelCost(char.level))}</small></>}
              </button>
            ...
            <div className="lvl-stack">
              ...
              <button className="btn side" data-ftue="equip-best" disabled={!canEquipBetter(state.gear, cid, cls)} onClick={doEquipBest}><Icon id="equipBest" /> EQUIP BEST</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="hm-sheet open">
            ...
            <div className="hm-card">
              ...
              <div className="eq-btns">
                ...
                <button className="btn primary" style={{ flex: 1 }} onClick={() => { const s = confirmAsc; setConfirmAsc(null); runAscend(s); }}><Icon id="ascend" /> Ascend</button>
              </div>
            </div>
          </div>
        );
      ...
      {/* ability detail */}
      {skillIdx != null && (() => {
        const A = [['ability.basic', 'Basic', ui.abilityNames?.basic || 'Attack', 'Standard attack on the focus target.'],
          ['ability.skill', 'Skill', ui.abilityNames?.normal || 'Skill', `${abilDesc(def.normal?.effect)} Charges every ${((def.normal?.chargeMs || 0) / 1000).toFixed(1)}s.`],
          ['ability.limit', 'Ultimate', ui.abilityNames?.limit || 'Ultimate', `${abilDesc(def.limit?.effect)} Fires after ${def.limit?.orders} orders.`]][skillIdx];
        return (
          <div className="hm-sheet open" onClick={() => setSkillIdx(null)}>
            ...
            <div className="hm-card" onClick={(e) => e.stopPropagation()}>
              <div className="skhead"><div className="sc"><Icon id={A[0]} /></div><div><div className="styp">{A[1]} · Lv {abilLv}</div><div className="snm">{A[2]}</div></div></div>
      ...
      {equipSlot && (
        <div className="hm-sheet open">
          ...
          <div className="hm-card eq-card">
            <button className="hm-x" onClick={() => setEquipSlot(null)}><Icon id="close" /></button>
            ...
            <div className="eq-cur">
              {cur ? <>
                ...
                <div className="pw"><Icon id="attack" /> {fmt(gearPower(cur))}</div>
              </> : <>
              ...
              {items.length ? items.map((g) => {
                ...
                return (
                  <div key={g.id} className={`eq-item${on ? ' on' : ''}`} style={{ '--gr': gColor(g.rarity) }}
                    ...
                    {on && <Icon id="check" className="eqbadge" />}
                    <span className="gi">{gearIcon(equipSlot)}</span>
                    <span className="pw"><Icon id="attack" /> {fmt(gearPower(g))}</span>
                    <span className={`d ${d >= 0 ? 'up' : 'dn'}`}>{on ? 'equipped' : (d >= 0 ? `▲ +${fmt(d)}` : `▼ ${fmt(Math.abs(d))}`)}</span>
                    {owner && <span className="owner"><Icon id="owner" />{HEROES[owner.hero].name[0]}</span>}
            ...
            <div className="eq-btns">
              <button className="btn primary" disabled={!cur || !canLevelGear(cur, state.gearXp)} onClick={doLevelGearOne}>
                {cur ? <>LEVEL UP<small><Art a={resolve('ui.gearXp')} /> {fmt(gearLevelCost(cur.level))}</small></> : 'LEVEL UP'}
              </button>
              <button className="btn b" disabled={!canEquipBetter(state.gear, cid, cls)} onClick={doEquipBest}><Icon id="equipBest" /> EQUIP BEST</button>
            </div>
          </div>
        </div>
      )}
      ...
      {confirmEq && (() => {
        ...
        const col = (item, label) => item
          ? <div className="ci" style={{ '--gr': gColor(item.rarity) }}><div className="cl">{label}</div><div className="cx">{gearIcon(slot)}<span className="clv">Lv{item.level}</span></div><div className="cn" style={{ color: gColor(item.rarity) }}>{item.rarity.toUpperCase()}</div><div className="cst"><Icon id="attack" /> {fmt(gearPower(item))}</div></div>
          : <div className="ci"><div className="cl">{label}</div><div className="cx"><Icon id="empty" /></div><div className="cn" style={{ color: 'var(--dim)' }}>Empty</div><div className="cst" style={{ color: 'var(--dim)' }}><Icon id="attack" /> 0</div></div>;
        ...
        return (
          <div className="hm-sheet open">
            ...
            <div className="hm-card">
              ...
              {owner && <div className="cmp-warn show"><Icon id="warn" /> Currently equipped by <b>{HEROES[owner.hero].name}</b>. Equipping will remove it from them.</div>}
              <div className="eq-btns">
