...
import { fxHeroLevelUp, fxMaxed, fxLevelAll, fxEquip, fxEquipBest, fxHeroFuse, fxCombatPow, fxSquadTotal } from '../fx/hero-fx.js';
...
export default function HeroesScreen() {
  ...
  useLayoutEffect(() => {
    ...
    // Squad-wide power-ups: fire the SAME in-tile fx per affected hero (level/gear burst + tile flash +
    // net power count-up) as the per-hero paths, for each squad member that actually changed.
    if (p.kind === 'squadlevel' || p.kind === 'squadgear') {
      let n = 0; let totalGain = 0; // stagger index (per animating hero, no gaps) + squad power delta
      for (const id in p.before) {
        const sh = state.heroes[id]; if (!sh) continue;
        const b = p.before[id]; const toP = powOf(id);
        const changed = p.kind === 'squadlevel' ? sh.level > b.fromLv : toP > b.fromPow;
        if (!changed) continue;
        totalGain += Math.max(0, toP - b.fromPow);
        const fire = () => {
          const t = tileEl(id); if (!t) return; // tile may be gone if the player navigated away mid-cascade
          if (p.kind === 'squadlevel') fxHeroLevelUp(t, { fromLv: b.fromLv, toLv: sh.level, fromPow: b.fromPow, toPow: toP, cap: heroMaxLevel(sh) });
          else { const gslots = [...document.querySelectorAll(`.hs-gslot[data-hero-id="${id}"]:not(.hs-empty)`)]; fxEquipBest(t, gslots, b.fromPow, toP); }
          fxCombatPow(id, toP - b.fromPow); // ALSO show the increase on the in-combat hero chip
        };
        if (n === 0) fire(); else setTimeout(fire, n * HS.squadStaggerMs); // slight per-hero delay → staggered cascade
        n++;
      }
      if (totalGain > 0) fxSquadTotal(totalGain); // squad power total feedback on the Heroes screen
      return;
    }
    const t = tileEl(p.id);
  ...
  // Squad-wide: capture EVERY squad member's before level+power, then dispatch — the layout effect fires
  // the in-tile fx for each hero that actually changed (identical to the per-hero buttons).
  const squadBefore = () => { const b = {}; for (const id of state.order.slice(0, SELECTED_SLOTS)) b[id] = { fromLv: state.heroes[id].level, fromPow: powOf(id) }; return b; };
  const doLevelUpSquad = () => { pending.current = { kind: 'squadlevel', before: squadBefore() }; actions.levelUpSquad(); };
  const doEquipBestSquad = () => { pending.current = { kind: 'squadgear', before: squadBefore() }; actions.equipBestSquad(); };
    ...
    return (
      <div className={`hs-cell ${active ? 'hs-active' : ''}`} key={id} data-hid={id} ref={(el) => { cellRefs.current[id] = el; }}>
        ...
        >
          ...
          <span className="hs-pow"><Icon id="attack" className="hs-ic" /><b>{fmt(power)}</b></span>
  ...
  // Squad power-up buttons: enabled only when SOME squad member can actually improve (never a dead tap).
  const canGearSquad = useMemo(() => squad.some((id) => canEquipBetter(state.gear, id, heroClassOf(state.heroes[id].hero))), [state.order, state.gear, state.heroes, state.ordersCompleted]); // eslint-disable-line react-hooks/exhaustive-deps
  const canLevelSquad = useMemo(() => squad.some((id) => canLevelHero(state.heroes[id], state.heroXp)), [state.order, state.heroes, state.heroXp]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    <div className={`hs-roster ${popId ? 'hs-ctx' : ''}`}>
      <div className="hs-head">
        ...
        <div className="hs-pools">
          <span className="hs-pool book"><Art a={resolve('ui.heroXp')} /> {fmt(state.heroXp)}</span>
          <span className="hs-pool wrench"><Art a={resolve('ui.gearXp')} /> {fmt(state.gearXp)}</span>
          <span className="hs-cnt">{ownedCount} / {total}</span>
        </div>
      </div>
      <PeekScroll>
        <div className="hs-sqhdr">
          <span className="hs-seclabel sq"><Icon id="rarity.star" /> Current Squad</span>
          <div className="hs-sqright">
            <button type="button" className="hs-sqbtn" data-ftue="gear-squad" disabled={!canGearSquad} onClick={doEquipBestSquad}>Best Gear</button>
            <button type="button" className="hs-sqbtn" data-ftue="level-squad" disabled={!canLevelSquad} onClick={doLevelUpSquad}>Level All</button>
            <span className="hs-sqpow"><Icon id="attack" className="hs-sqpow-ic" />{fmt(squadPower)}</span>
          </div>
        </div>
          ...
          <>
            ...
            <div className="ability-pop" role="dialog" aria-label="Confirm ascension">
              ...
              <div className="hs-pop-btns">
                <button type="button" className="hs-pop-btn green" onClick={() => { const s = confirmAsc; setConfirmAsc(null); performAscend(s); }}><Icon id="ascend" /> Ascend</button>
    ...
    <div ref={popRef} className="hs-pop" style={{ left: pos.x, top: pos.y }} onClick={(e) => e.stopPropagation()}>
      <div className="hs-pop-head">
        ...
        <div>
          ...
          <div className="hs-ph-sub">Lv {st.level}/{cap} · <b><Icon id="attack" /> {fmt(powOf(id))}</b></div>
      ...
      {popState === 'hero' ? (
        <div className="hs-pop-btns">
          <button type="button" className="hs-pop-btn gold" disabled={!canLvl && !atMax} onClick={() => onLevelUp(id)}>
            {atMax ? <><Icon id="rarity.star" /> Max Level</> : <><Icon id="levelUp" /> Level Up</>}
          </button>
          <button type="button" className="hs-pop-btn gold" disabled={!canLvlMax} onClick={() => onLevelUpMax(id)}><Icon id="levelUpMax" /> Level Up Max</button>
          ...
          {canAscend && (
            <button type="button" className="hs-pop-btn green" onClick={() => onAscend(id)}>
              <Icon id="ascend" /> Ascend {asc}/{HERO_UPGRADE.maxAscensions}
              <span className="hs-asc-req"> <Icon id="sacrifice" /> sacrifices 1 copy</span>
            </button>
          )}
          <button type="button" className="hs-pop-btn blue" onClick={() => setPopState('equip')}><Art a={resolve('gear.armor')} /> Equipment</button>
        </div>
      ) : popState === 'equip' ? (
        <div className="hs-pop-btns">
          <button type="button" className="hs-pop-btn blue" onClick={() => { setPopState('reequip'); setSelSlot(null); }}><Icon id="reequip" /> Reequip</button>
          <button type="button" className="hs-pop-btn green" disabled={!canUpg} onClick={() => onLevelAll(id, false)}>＋ Level All +1</button>
          <button type="button" className="hs-pop-btn green" disabled={!canUpg} onClick={() => onLevelAll(id, true)}><Icon id="levelUpMax" /> Level All Max</button>
    ...
    <>
      ...
      {selSlot ? <>
        <div className="hs-cmp-cur">
          {cur
            ? <>Equipped: <b style={{ color: gearColor(cur.rarity) }}>{GEAR_RARITY[cur.rarity].name}</b> · <Icon id="attack" /> {fmt(gearPower(cur))}</>
          ...
          {cands.length ? cands.map((g) => {
            ...
            return (
              <button key={g.id} type="button" className="hs-cmp-item" style={{ '--gr': gearColor(g.rarity) }} onClick={() => onEquipItem(id, selSlot, g.id)}>
                ...
                <span className="hs-ci-pw"><Icon id="attack" /> {fmt(gearPower(g))}</span>
      ...
      <div className="hs-pop-btnrow">
        ...
        <button type="button" className="hs-pop-btn blue" disabled={!canBetter} onClick={() => onEquipBest(id)}><Icon id="equipBest" /> Equip Best</button>
      </div>
    </>
  );
}
