...
// FTUE order-slot ramp — the guided opening starts with C.FTUE.startingOrderSlots order slots and grows
// by one per completed order up to the normal ORDER_CONFIG.active. Off-FTUE → always the full active count
// (removal contract). SINGLE source of the slot count, read by initState + FULFILL_ORDER.
export const orderSlotTarget = (ftueActive: boolean, ordersCompleted: number): number =>
  (!ftueActive || !C.FTUE) ? C.ORDER_CONFIG.active
    : Math.min(C.ORDER_CONFIG.active, Math.max(C.FTUE.startingOrderSlots, ordersCompleted));

// The FTUE's forced "good gear" order (the guided blade order → epic armour via forceSlot). Built here so
// the first gap-fill refill (reducer-orders) constructs it identically to the old opening. Only called
// while the FTUE is active, so C.FTUE is present.
export const buildFtueGearOrder = (id: number) => {
  const F = C.FTUE!;
  const items = [{ chain: F.secondOrderChain, level: F.secondOrderTier }];
  return { id, items, difficulty: Orders.tileTotal(items), rarity: F.secondOrderGearRarity, reward: F.secondOrderReward, forceSlot: F.secondOrderGearSlot };
};

// (Re-)arm the FTUE gear-up guide (go to HEROES → Best Gear Squad → Level Up Squad). Called on the
// good-gear chest pop AND on each qualifying loss: sets the armed flag, resets the "tapped" flags, and
// clears the chain's per-step seen flags so the coachmarks re-show. Shared by reducer-combat + -orders.
export const armGearGuide = (flags: any) => ({
  ...flags, ftueGearArmed: true, ftueSquadGeared: false, ftueSquadLeveled: false,
  ftueSeen_gearOpen: false, ftueSeen_gearGearSquad: false, ftueSeen_gearLevelSquad: false,
});
...
export const initState = (now: number, saved: any = null): S => {
  ...
  // FTUE paces the opening: start with orderSlotTarget(…, 0) slots (1) and grow per completion; off-FTUE = full active count.
  const openSlots = orderSlotTarget(ftueOn, 0);
  ...
  // FTUE override layer (fresh save only): force the OPENING order [0] to the keystone Limit Potion — the
  // ONLY order the guided run opens with (openSlots = 1). The good-gear blade order arrives as the FIRST
  // refill after it (reducer-orders FILL_ORDER_GAP → buildFtueGearOrder); slots then ramp per completion.
