...
export const boardHandlers: Record<string, (state: S, action: Act) => S> = {
  [A.TAP_GENERATOR]: (state, action) => {
    ...
    // FTUE: arm the "you forged a tile" coachmark flag (monotonic; inert when the FTUE is off). Set on a
    // SUCCESSFUL forge only — the board seeds tiles, so tile-count alone can't detect the tap. Mirrors the
    // ftueLimitFired hook in reducer-combat.ts.
    const flags = (state.flags && state.flags.ftueActive && !state.flags.ftueForged) ? { ...state.flags, ftueForged: true } : state.flags;
    return { ...state, flags, board, energy, fx, nextId: id, now: action.now };
  },
