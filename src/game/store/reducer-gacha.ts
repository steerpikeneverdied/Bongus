...
export const gachaHandlers: Record<string, (state: S, action: Act) => S> = {
  [A.SUMMON]: (state, action) => {
    ...
    if (state.flags && state.flags.ftueFirstPull && C.FTUE && C.FTUE.firstPullHero) {
      ...
      // Record the recruit's cid so the `alchemistUse` coachmark can spotlight ITS limit button
      // (the potion charges several heroes, so the beat can't just target "any charged limit").
      const flags = { ...state.flags, ftueFirstPull: false, ftuePulled: true, ftueAlchemistCid: cid };
