...
import { heroPower, heroRarity } from '../heroes/heroes.ts';
import { C } from '../content.ts';
import { A } from './actions.ts';
import { battleStatsFor, rescaleBattle, squadOf } from './reducer-helpers.ts';
...
export const gearHandlers: Record<string, (state: S, action: Act) => S> = {
  ...
  // Squad-wide best-gear: BEST RARITY heroes first, then downwards (power breaks ties) — so the rarest
  // hero gets first pick of the shared pool. AUTO_EQUIP scoped to the squad.
  [A.EQUIP_BEST_SQUAD]: (state) => {
    const rarityTier = (cid: string) => (C.HERO_RARITIES[heroRarity(state.heroes[cid]) || 'common'] || {}).tier ?? 0;
    const pow = (cid: string) => heroPower(state.heroes[cid].hero, state.heroes[cid], state.ordersCompleted, 0);
    const ranked = [...squadOf(state.order)].sort((x: string, y: string) => (rarityTier(y) - rarityTier(x)) || (pow(y) - pow(x)));
    const gear = Gear.autoEquipAll(state.gear, ranked.map((cid: string) => ({ id: cid, cls: Gear.heroClassOf(state.heroes[cid].hero) })));
    // FTUE: mark the squad-gear lesson done (drives the loss guide's auto-dismiss). Inert off-FTUE.
    const flags = (state.flags && state.flags.ftueActive && !state.flags.ftueSquadGeared) ? { ...state.flags, ftueSquadGeared: true } : state.flags;
    if (gear === state.gear) return flags === state.flags ? state : { ...state, flags };
    const statsFn = battleStatsFor(state.heroes, gear, state.ordersCompleted);
    return { ...state, flags, gear, battle: { ...state.battle, heroes: rescaleBattle(state.battle, statsFn) } };
  },
