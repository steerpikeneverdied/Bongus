...
import { respawn, winReward, rollWinCrystal, addCrystal, armGearGuide } from './reducer-helpers.ts';
...
export const combatHandlers: Record<string, (state: S, action: Act) => S> = {
  ...
  [A.BATTLE_TICK]: (state, action) => {
    ...
    if (outcome === 'lose') {
      ...
      // FTUE: re-arm the gear-up guide on the FIRST TWO losses (go HEROES → Best Gear Squad → Level Up
      // Squad). ftueGearGuideCount caps it at 2; 3rd+ loss does nothing. Inert when the FTUE is off.
      const lostFlags = (state.flags && state.flags.ftueActive && (state.flags.ftueGearGuideCount || 0) < 2)
        ? { ...armGearGuide(state.flags), ftueGearGuideCount: (state.flags.ftueGearGuideCount || 0) + 1 }
        : state.flags;
  ...
  [A.TAP_LIMIT]: (state, action) => {
    // FTUE: the FIRST guided limit break is a guaranteed CRIT that instantly wipes the wave.
    const critKill = !!(state.flags && state.flags.ftueActive && !state.flags.ftueLimitFired);
    const { battle, outcome, fired, enemyDamage, enemyDeaths, heals, combo, crit } = Battle.fireLimitBreak(state.battle, action.heroId, critKill);
    if (!fired.length) return state;
    let id = state.nextId;
    let fx = [...state.fx, { id: id++, type: 'limitBreak', heroes: fired, heroId: action.heroId, enemyDamage, enemyDeaths, heals, crit }];
    if (combo) fx = [...fx, { id: id++, type: 'combo', uid: combo.uid, n: combo.n }];
  ...
  [A.RESOLVE_WIN]: (state) => {
    ...
    if (toZone > fromZone && state.battle.level > state.clearedLevel) {
      ...
      // Zone-completion generator award — data-driven per zone (each zone's `rewardGenerators`). Starting
      // blade-only, Area 1 awards range + Area 2 magic (the new unlocks); later areas award duplicates.
    ...
    if (Map.isBossLevel(nextLevel)) {
      // Recovering: a pre-boss win does NOT advance to the boss — REPEAT the current level (stay
      // recovering) until the player clicks the BOSS button (CHALLENGE_NEXT). A normal win gates at the boss.
      if (state.battle.recovering) {
        const { wave, heroes, id: id3 } = respawn(state, state.battle.level, state.nextId);
        return { ...state, battle: { ...state.battle, wave, heroes, status: 'intro', recovering: true }, nextId: id3, clearedLevel: Math.max(state.clearedLevel, state.battle.level) };
      }
