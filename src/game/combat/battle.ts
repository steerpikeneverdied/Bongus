...
export const fireLimitBreak = (battle: BattleState, cid: string, critKill = false) => {
  ...
  wave = aoeDamage(wave, acc.aoe);
  wave = focusDamage(wave, acc.focus, battle.focusUid ?? null);
  // FTUE guided first limit break: a guaranteed CRIT that instantly wipes the wave (a big, satisfying
  // first payoff). Off by default → normal damage. Set only by TAP_LIMIT for the first FTUE limit.
  if (critKill) wave = wave.map((e) => (e.hp > 0 ? { ...e, hp: 0 } : e));
  const enemyDamage = wave.filter((e) => (wavePre.get(e.uid) as number) > e.hp).map((e) => ({ uid: e.uid, amount: (wavePre.get(e.uid) as number) - e.hp }));
  const enemyDeaths = wave.filter((e) => (wavePre.get(e.uid) || 0) > 0 && e.hp <= 0).map((e) => e.uid);
  const outcome = allDead(wave) ? 'win' as const : null;
  return { battle: { ...battle, heroes, wave, comboUid, comboN, focusUid: survivingFocus(battle.focusUid, wave) }, outcome, fired: [cid], enemyDamage, enemyDeaths, heals, combo, crit: critKill };
};
