...
`dev-pause.ts` is a dev-only mutable flag (not game state) that REGEN_TICK/BATTLE_TICK also check
alongside the hero-menu/AFK guard — set by the Marksman markup tool's `GameAdapter.setPaused`
(`main.tsx`) so the sim holds still while annotating a frame. Always `false` in a shipped build
(Marksman only mounts behind `import.meta.env.DEV`).
