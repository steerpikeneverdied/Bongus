...
import { z } from 'zod';
import { zConfig, configRef, keyConfigRef, configRecord } from '@fortis/bishop-config-registry';
...
export const zZoneConfig = zConfig
  .extend({
    ...
    crystalRarityKey: keyConfigRef('rarities').describe('Which ascension-crystal tier drops here (→ rarities.key).'),
    ...
    rewardGenerators: z.array(z.object({
      generatorKey: keyConfigRef('generators').describe('Which generator (→ generators.key).'),
