...
export const zGearPieceConfig = zConfig
  .extend({
    slot: keyConfigRef('gearSlots').describe('Equip slot family (→ gearSlots.key).'),
    maxRarityKey: keyConfigRef('gearRarities').describe('Highest rarity this piece can roll/fuse to (→ gearRarities.key).'),
    classKey: keyConfigRef('heroClasses').optional().describe('Set for a class-bound piece (a classAccessory) — it only fits a hero-class whose classKey matches (→ heroClasses.key).'),
