...
import { z } from 'zod';
import { configRef, keyConfigRef, configRecord } from '@fortis/bishop-config-registry';
...
export const zBoardConfig = z.object({
  ...
  startLayout: z.object({
    startingGeneratorKeys: z.array(keyConfigRef('generators')).default([]).describe('Generators UNLOCKED at boot (→ generators.key). Others in `generators` are placed only once their unlocking area is cleared. Drives board placement + order eligibility.'),
    generators: z.array(z.object({ generator: keyConfigRef('generators'), cell: int }).strict()).describe('Board cell layout for EVERY generator (→ generators.key); only currently-unlocked ones are placed.'),
    seedItems: z.array(z.object({
      chain: keyConfigRef('chains'), level: int, cell: int, locked: z.boolean().optional(),
    }).strict()),
  }).strict(),
}).strict();
...
export const zOrdersConfig = z.object({
  ...
  orderChains: z.array(keyConfigRef('chains')),
...
export const zGearTuningConfig = z.object({
  ...
  chestTiers: z.array(z.object({
    ...
    rarity: keyConfigRef('gearRarities'),
  }).strict()),
}).strict();
...
export const zGearLoadoutConfig = z.object({
  defaultSlots: z.array(keyConfigRef('gearSlots')).describe('Ordered slot keys every hero-class has unless it overrides `slots` (→ gearSlots.key).'),
}).strict();
...
export const zAnimConfig = z.object({
  ...
  heroes: z.object({
    ...
    squadStaggerMs: num.describe('Per-hero delay (ms) between squad-wide level-up / best-gear in-tile VFX, for a staggered cascade.'),
...
export const zVfxConfig = z.object({
  ...
  combat: z.object({
    ...
    loseDrips: z.object({
      inset: num.describe('Emission-line boundary inset (px) in from each end of the LOSE word.'),
      startY: num.describe('Emission start Y as % into the glyph height (from top).'),
      emitMs: num.describe('Interval (ms) between emission ticks.'),
      countPerTick: int.describe('Blobs emitted per tick, spread across the line between the boundaries.'),
      arcDeg: num.describe('Downward launch cone (deg), centred on straight-down.'),
      distMin: num, distMax: num.describe('Blob drip distance range (px).'),
      blobPx: num.describe('Blob diameter (px).'),
      flyMs: num.describe('Drip duration (ms): falls + scales to 0 (no fade).'),
      popMs: num.describe('Scale-in duration (ms): popScale→1 before the drip.'),
      popScale: num.describe('Blob start scale (0..1) before the pop-in.'),
    }).strict().describe('LOSE-screen red drip emitter fired from the LOSE glyphs (limit-blob style, downward).'),
