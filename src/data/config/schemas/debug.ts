...
export const zDebugConfig = z.object({
  ...
  currencyGrant: z.number().int().min(0).describe('Amount of EACH upgrade currency (coins, heroXp, gearXp, every crystal rarity) granted by the giveCurrency debug feature (the S hotkey).'),
  features: z.array(z.object({
    key: z.string().describe('Stable key the runtime gates on (debugFeatureOn).'),
    label: z.string().describe('Short human label.'),
    description: z.string().describe('What the feature does + how it is triggered.'),
    defaultOn: z.boolean().describe('Whether this feature is active while debug is enabled.'),
  }).strict()).describe('Every debug feature, each schema-validated. Add one here + gate its code on debugFeatureOn(key).'),
}).strict();
