...
import { z } from 'zod';
import { configRef, keyConfigRef } from '@fortis/bishop-config-registry';
...
export const zFtueConfig = z.object({
  ...
  firstOrderChainKey: keyConfigRef('chains').describe('The chain the opening order requests — = the tile the guided forge+merge produces (→ chains.key).'),
  ...
  secondOrderChainKey: keyConfigRef('chains').describe('The chain the second order requests — the tile the player merges to fulfil it (→ chains.key).'),
  secondOrderTier: z.number().int().min(0).describe('The 0-based tier the second order requests.'),
  secondOrderGearSlot: keyConfigRef('gearSlots').describe('The gear SLOT the forced second-order reward grants (→ gearSlots.key), e.g. armor.'),
  secondOrderGearRarity: keyConfigRef('gearRarities').describe('The rarity of the forced second-order gear reward (→ gearRarities.key), e.g. epic — capped at the rolled piece’s maxRarity.'),
  ...
  startingOrderSlots: z.number().int().min(1).describe('The FTUE opens with this many order slots; +1 per completed order up to ORDER_CONFIG.active (paces the guided opening so it can’t be rushed).'),
}).strict();
