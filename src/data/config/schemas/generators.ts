...
import { z } from 'zod';
import { zKeyConfig, keyConfigRef } from '@fortis/bishop-config-registry';
...
export const zGeneratorConfig = zKeyConfig
  .extend({
    chainKey: keyConfigRef('chains').describe('The chain this generator dispenses (→ chains.key).'),
    weapon: keyConfigRef('chains').describe('VFX trail/weapon type when an order bashes apart (→ chains.key).'),
