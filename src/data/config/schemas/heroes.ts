...
import { z } from 'zod';
import { zConfig, keyConfigRef } from '@fortis/bishop-config-registry';
...
const zEffect = z.object({
  ...
  statusKeys: z.array(keyConfigRef('statuses')).optional().describe('Status RIDER: statuses this effect applies (→ statuses.key), alongside any primary.'),
...
export const zHeroConfig = zConfig
  .extend({
    rarityKey: keyConfigRef('rarities').describe('Rarity tier (→ rarities.key).'),
    weaponChainKey: keyConfigRef('chains').describe('Weapon/merge chain this hero wields (→ chains.key); picks the trail colour.'),
    ...
    classKey: keyConfigRef('heroClasses').describe("This hero-class's class (→ heroClasses.key); gates class-bound equip slots (the class accessory)."),
    ...
    slots: z.array(keyConfigRef('gearSlots')).optional().describe('The equip loadout THIS class has (→ gearSlots.key), ordered. Omit to inherit gearLoadout.defaultSlots.'),
  })
  .strict();
