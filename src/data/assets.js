...
export const ASSETS = {
  ...
  // ── Icons (formerly hardcoded emoji literals) ───────────────────────────────
  // Every glyph that used to be inlined in components / strings / CSS / the minigame is registered
  // here so it flows through the trim tool's "Icons" section (generate + export) and renders via
  // <Art>/resolve() like everything else — no emoji baked into view code. `art` is filled in on
  // export (icons/<slug>.png); until then the emoji is the resolver fallback. The trim tool's
  // build-icon-manifest.mjs is a deduped VIEW over these + the ui.*/status.*/node.*/fx.*/banner.*/
  // gear.* entries above (glyph shared across ≥2 categories → the "general" bucket).
  // Hero abilities (hero menu tabs + ability rows).
  'icon.ability.basic': { emoji: '👊', label: 'Basic' },
  'icon.ability.skill': { emoji: '✨', label: 'Skill' },
  'icon.ability.limit': { emoji: '💥', label: 'Ultimate' },
  // Stat rows (hero menu).
  'icon.stat.hp': { emoji: '❤️', label: 'Health' },
  'icon.stat.atk': { emoji: '🗡️', label: 'Attack' },
  'icon.stat.def': { emoji: '🛡️', label: 'Defense' },
  'icon.power': { emoji: '✊', label: 'Power' },
  'icon.attack': { emoji: '⚔️', label: 'Attack Power' },   // the ⚔ power/might prefix in the hero HUD
  'icon.focus': { emoji: '🎯', label: 'Focus' },
  'icon.owner': { emoji: '👤', label: 'Owner' },
  // UI chrome (buttons / states across screens).
  'icon.close': { emoji: '✕', label: 'Close' },
  'icon.check': { emoji: '✓', label: 'Confirm' },
  'icon.lock': { emoji: '🔒', label: 'Locked' },
  'icon.levelUp': { emoji: '⬆️', label: 'Level Up' },
  'icon.levelUpMax': { emoji: '⏫', label: 'Level Up Max' },
  'icon.ascend': { emoji: '✦', label: 'Ascend' },
  'icon.reequip': { emoji: '♻️', label: 'Reequip' },
  'icon.sacrifice': { emoji: '🔁', label: 'Sacrifice' },
  'icon.equipBest': { emoji: '⚡', label: 'Equip Best' },
  'icon.empty': { emoji: '➖', label: 'Empty' },
  'icon.warn': { emoji: '⚠️', label: 'Warning' },
  // Orders.
  'icon.potion': { emoji: '🧪', label: 'Potion' },
  // Board / merge surface.
  'icon.generator': { emoji: '⚙️', label: 'Generator' },
  'icon.generatorBadge': { emoji: '⚡', label: 'Generator Badge' },
  'icon.cobweb': { emoji: '🕸️', label: 'Locked Tile' },
  'icon.bestMerge': { emoji: '✨', label: 'Best Merge' },
  // Combat / boss banners.
  'icon.boss': { emoji: '⚔️', label: 'Boss' },
  // AFK / away.
  'icon.afk': { emoji: '💤', label: 'AFK' },
  'icon.skull': { emoji: '☠️', label: 'Skull' },
  // Gacha reveal rarity marks.
  'icon.rarity.star': { emoji: '★', label: 'Rarity Star' },
  'icon.rarity.diamond': { emoji: '◆', label: 'Rarity Diamond' },
  // Minigame HUD (canvas-drawn).
  'icon.minigame.bomb': { emoji: '💣', label: 'Bomb' },
  'icon.minigame.squad': { emoji: '🏹', label: 'Squad' },
  'icon.minigame.coins': { emoji: '⛁', label: 'Minigame Coins' },
  // Dev-only chrome (guarded by import.meta.env.DEV).
  'icon.headless': { emoji: '☾', label: 'Background Mode' },
  'icon.dev.minigame': { emoji: '🎮', label: 'Test Minigame' },
  'icon.dev.perf': { emoji: '📋', label: 'Copy Perf' },
