...
```
   ...
   @fortis/bishop-device-preview — dev-only device-frame preview (npm devDep; not part of the sim layers)
   @fortis/bishop-marksman       — dev-only markup/feedback overlay + capture endpoint (npm devDep; not part of the sim layers)
```
...
### 4. `DevicePreviewFrame` — dev-only device-frame preview (`@fortis/bishop-device-preview`)

A React component (consumed as a published npm devDependency) that wraps the running page in an
iframe-based phone/tablet bezel with a device picker and safe-area overlay, so you can eyeball real
device dimensions from a desktop browser. `mountDevicePreview()` is called from `src/main.tsx` behind
an `import.meta.env.DEV` guard — it never reaches a production build. Unlike the other three
primitives it isn't part of the simulation's dependency chain (`data → game → {view, ui}`); it's pure
dev tooling, exempt from the DI/signals/data-values rules. See the package README.

### 5. `Marksman` — dev-only markup / feedback overlay (`@fortis/bishop-marksman`)

A React overlay (consumed as a published npm devDependency) that floats over the running game so you
...
overlay (`mountMarksman`, from `src/main.tsx` behind an `import.meta.env.DEV` guard) and a dev-server
endpoint (`@fortis/bishop-marksman/endpoint`, a Vite plugin `apply: 'serve'` in `vite.config.ts`)
that does the file writes — both dev-only, never in a production build. Like the preview it sits
outside the sim dependency chain, exempt from the DI/signals/data-values rules. The one game-specific
seam is the `GameAdapter` (only `setPaused` required). An optional voice feature adds whisper.cpp
transcription: it's off by default (`marksman.config.json` `features.audio`), and when enabled
`vite.config.ts` wires the package's `@fortis/bishop-marksman/features/audio/server` routes
(`ffmpeg-static` devDep + a local whisper.cpp install required). See the package README.
