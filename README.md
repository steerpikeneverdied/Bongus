...
real `@fortis/bishop-*` engine (published npm packages), wired to the three-registry data-layer
...
```bash
npm install        # pulls @fortis/bishop-* from the Fortis registry (needs ~/.npmrc auth)
npm run dev        # boots the placeholder; the boot screen shows a data-source marker
npm run build      # tsc → vite build (the full gate)
```
...
| Account | `src/account` | the six-section economy blob (`@fortis/bishop-meta-contract`) |
...
`@fortis/bishop-*` are the real engine packages (config-registry, asset-registry, asset-types-2d,
asset-processors, render-contract, meta-contract, meta-client), consumed as published npm
dependencies (`^1.0.0`) from the Fortis registry. `meta-engine` (the server backend) is intentionally
not a dependency — the account is local until the economy ports to a server (a wiring change).
