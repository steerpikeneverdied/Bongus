...
import marksmanEndpoint from '@fortis/bishop-marksman/endpoint';
import { audioRoutes } from '@fortis/bishop-marksman/features/audio/server';
import { configEditorEndpoint } from './src/config-editor/endpoint.mjs';

// The data-driven engine wiring (@fortis/bishop-*):
//  - virtual:game-config    ← the merged, validated logical config + singletons + UI + visual registries.
//  - virtual:asset-registry ← the resolved asset database (ids → urls); formats from @fortis/bishop-asset-types-2d.
import { gameConfigPlugin } from '@fortis/bishop-config-registry/vite';
import { assetRegistryPlugin } from '@fortis/bishop-asset-registry/vite';
import { assertValidMerged, nextIds } from '@fortis/bishop-config-registry';
import { readLedger } from '@fortis/bishop-config-registry/node';
import { scanAssetsDir } from '@fortis/bishop-asset-registry/node';
import '@fortis/bishop-asset-types-2d'; // register asset extractors so the compose asset-ref integrity check can scan
...
// Marksman voice/audio feature is opt-in via marksman.config.json `features.audio`. When on, wire the
// package's transcription server routes (@fortis/bishop-marksman/features/audio/server); whisper.cpp +
// ffmpeg-static must be provisioned or the routes report available:false gracefully.
  ...
  plugins: [
    ...
    assetRegistryPlugin({
      ...
      registerFormats: async () => {
        await import('@fortis/bishop-asset-types-2d');
        await import('@fortis/bishop-asset-types-2d/schema');
      },
    }),
    // Dev-only Marksman capture endpoint (@fortis/bishop-marksman/endpoint). Defaults match this repo
    // (marksman.config.json + .cache/markdown). Audio transcription routes wired when features.audio is on.
