...
// A config category's `_default.json` (partial config deep-merged UNDER every entity) IS schema-
// checked below (via validateCategoryDefault — known-field check, not a full-entity parse). Every
// OTHER `_`-prefixed file (_global, _id-ledger, _<singleton>) isn't a per-entity schema file —
// leave those to the build gate + `config validate`. Assets manifests are always "assets.json".
const isCategoryDefault = isConfig && basename(rel) === '_default.json';
if (!isAssets && !isCategoryDefault && basename(rel).startsWith('_')) pass();
...
try {
  if (isCategoryDefault) {
    // Partial per-category default: every top-level key must be a KNOWN field of the category schema
    // (catches typos). Type/nested checks stay on the MERGED entity — a partial can't be full-parsed.
    const cat = rel.split('/')[4]; // src/data/config/game/<cat>/_default.json
    const { CATEGORIES } = await import(resolve(ROOT, 'src/data/config/manifest.ts'));
    const c = CATEGORIES.find((x) => x.folder === cat || x.name === cat);
    if (!c) pass(); // unknown folder — not a category
    const { validateCategoryDefault } = await import('@fortis/bishop-config-registry');
    const issues = validateCategoryDefault(c.schema, data);
    if (issues.length) block(`config/${cat} ${rel} (_default):\n  ${issues.join('\n  ')}`);
  } else if (isConfig) {
    ...
    await import('@fortis/bishop-asset-types-2d/schema'); // register the 2D asset schemas
    const { validateManifest } = await import('@fortis/bishop-asset-registry');
