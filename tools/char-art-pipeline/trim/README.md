...
- `bash launch_tool.sh` — opens <http://localhost:8791>.

> Bishop-idle-merge's trim tool runs on **port 8791** with its own `/tmp/bishop_*` state — fully
> separate from the combatclean tool (port 8790). `TrimTool.app` launches this repo's `launch_tool.sh`.
