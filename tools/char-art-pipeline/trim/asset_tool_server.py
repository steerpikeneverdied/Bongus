...
Run:  python3 asset_tool_server.py                 # root = ./assets, port 8791
      python3 asset_tool_server.py --root assets --port 8791
Then open http://localhost:8791
(Bishop-idle-merge's own port + /tmp/bishop_* paths — fully separate from the combatclean tool on 8790.)
"""
...
ICON_PIPELINE = os.path.join(os.path.dirname(PIPELINE), "icon-pipeline")         # sibling icon gen pipeline (icons-<cat>)
def is_icon_cat(c): return str(c or "").startswith("icons-")                     # Icons section categories
...
def run_export(no_build, dry=False, category=None):
    ...
    if category in MERGE_CATS:
        ...
        if is_icon_cat(category): extra += " --icons"   # icons export path in export-to-game.mjs
...
EXPORT_LOG = "/tmp/bishop_export.log"
EXPORT = {"proc": None}
...
def start_export(no_build, category=None):
    ...
    if category in MERGE_CATS:
        ...
        extra = (" --no-build" if no_build else "") + (" --icons" if is_icon_cat(category) else "") + _cat_flag(category)
...
def _regen_plan(root, category):
    ...
    elif is_icon_cat(category):                       # icons-<cat> → icon pipeline (roster per category)
        tsv = "rosters/%s.tsv" % category; env["TSV"] = tsv; env["OUT"] = os.path.join(root, category); cwd = ICON_PIPELINE
...
VARIANTS_DIR = "/tmp/bishop_variants"
...
def main():
    ...
    ap.add_argument("--port", type=int, default=8791)   # bishop-idle-merge's own port (combatclean uses 8790)
    args = ap.parse_args()
