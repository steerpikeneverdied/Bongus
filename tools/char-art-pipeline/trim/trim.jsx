...
(function () {
  // ---- defaults ----
  var ROOT        = "/Users/simonhill/bishop-idle-merge/tools/char-art-pipeline/trim/assets";
  ...
  var SIZE_256    = 256;
  var RUNCTL      = "/Users/simonhill/bishop-idle-merge/tools/char-art-pipeline/trim/trim_run.json";
    ...
    for(var i=0;i<files.length;i++){
      ...
      for(var si=0; si<sizes.length; si++){
        ...
        try{
          ...
          var room=sz-2*(totalWidth(treat)+2);          // leave space for the outline (none for icons)
          if(room<8) room=sz-4;
          ...
          applyTreatment(dup,dup.artLayers[0],treat);
          dup.saveAs(new File(cat.fsName+"/"+name.replace(/\.png$/i,"_"+suffix+".png")),png,true,Extension.LOWERCASE);
          exported++;
      ...
      // C+D. large size = full canvas, large treatment — SKIPPED for icons (smallest-only)
      if(!isIconCat){
