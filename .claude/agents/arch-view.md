...
5. Canvas sizing: the renderer is sized to the canvas's own CSS box (the `#game-frame`), driven by a `ResizeObserver` in `src/app/game-app.ts` — NOT to `window.innerWidth`/`window.innerHeight`. Flag any renderer/`resize()` caller that sizes the canvas or its backing store from the window instead of the canvas's client box (this breaks correctness whenever `#game-frame` ≠ the window, e.g. under the dev-only `@fortis/bishop-device-preview` device-frame wrapper).
